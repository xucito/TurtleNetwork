...
class BurnTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("non-issuer able to burn assets that he own") {
    for (v <- supportedVersions) {
      ...
      sender.grpc.broadcastTransfer(firstAcc, Recipient().withAddress(secondAddress), transferredQuantity, minFee, assetId = issuedAssetId, waitForTx = true)
      ...
      assertGrpcError(
        sender.grpc.broadcastTransfer(secondAcc, Recipient().withAddress(firstAddress), transferredQuantity, minFee, assetId = issuedAssetId),
