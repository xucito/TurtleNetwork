...
class DataTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("try to send tx above limits of key, value and size") {
    ...
    assertGrpcError(sender.putData(firstAcc, List(DataEntry("", DataEntry.Value.BoolValue(false))), 1.TN), "Empty key found", Code.INTERNAL)
    assertGrpcError(
      sender.putData(firstAcc, List(DataEntry("abc", DataEntry.Value.BoolValue(false)), DataEntry("abc", DataEntry.Value.BoolValue(false))), 1.TN),
