...
class DataTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("try to send tx above limits of key, value and size") {
    ...
    assertGrpcError(sender.grpc.putData(firstAcc, List(DataEntry("", DataEntry.Value.BoolValue(false))), 1.TN), "Empty key found", Code.INTERNAL)
    assertGrpcError(
      sender.grpc.putData(firstAcc, List(DataEntry("abc", DataEntry.Value.BoolValue(false)), DataEntry("abc", DataEntry.Value.BoolValue(false))), 1.TN),
  ...
  test("try to make address with 1000 DataEntries") {
    ...
    val txIds = dataSet.grouped(100).map(_.toList).map(data => PBTransactions.vanilla(sender.grpc.putData(thirdAcc, data, calcDataFee(data))).explicitGet().id().base58)
