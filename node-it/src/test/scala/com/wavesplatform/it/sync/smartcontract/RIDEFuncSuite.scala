...
class RIDEFuncSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("assetBalance() verification") {
    ...
    sender.transfer(acc0.stringRepr, newAddress, 10.TN, minFee, waitForTx = true)
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        TransferTransactionV2
          .selfSigned(Waves, pkNewAddress, pkNewAddress, 1.TN, System.currentTimeMillis(), Waves, smartMinFee, Array())
          .explicitGet()
          .json()),
  ...
  test("lastBlock and blockInfoByHeight(last) must return liquid block") {
    ...
    val transfer = sender.transfer(newAddress, newAddress, 1.TN, minFee + (2 * smartFee))
    nodes.waitForHeightAriseAndTxPresent(transfer.id)
  }
}
