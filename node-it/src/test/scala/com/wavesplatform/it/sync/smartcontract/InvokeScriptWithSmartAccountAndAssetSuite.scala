...
class InvokeScriptWithSmartAccountAndAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_send TN to dApp and caller accounts") {
    val dAppTransferId        = sender.transfer(sender.address, dApp.stringRepr, 5.TN, minFee).id
    val callerTransferId      = sender.transfer(sender.address, caller.stringRepr, 5.TN, minFee).id
    val smartCallerTransferId = sender.transfer(sender.address, smartCaller.stringRepr, 5.TN, minFee).id
