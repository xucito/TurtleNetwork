...
class InvokeScriptTransactionStateChangesSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("prepare") {
    ...
    sender.massTransfer(contract, List(Transfer(callerAddress, 3000), Transfer(recipientAddress, 3000)), 0.04.TN, assetId = Some(simpleAsset))
    sender.massTransfer(
      contract,
  ...
  test("state changes order") {
    val script = ScriptCompiler
      ...
      .bytes()
      .base64
    sender.setScript(contract, Some(script), setScriptFee + 0.4.TN, waitForTx = true)
