      ...
      "transfer tx sponsored fee is less then new minimal" in {
        ...
        assertBadRequestAndResponse(
          sender
            ...
            .id,
          s"Fee for TransferTransaction \\($SmallFee in ${Some(secondSponsorAssetId).get}\\) does not exceed minimal value of 2000000 TN or $LargeFee ${Some(secondSponsorAssetId).get}"
        )
      }
