...
class RIDEFuncSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("assetBalance() verification") {
    ...
    sender.transfer(acc0.address, newAddress, 10.TN, minFee, waitForTx = true)
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        TransferTransactionV2
          .selfSigned(None, pkNewAddress, pkNewAddress, 1.TN, System.currentTimeMillis(), None, smartMinFee, Array())
          .explicitGet()
          .json()),
    ...
    sender.signedBroadcast(
      TransferTransactionV2
        .selfSigned(Some(ByteStr.decodeBase58(asset).get), acc0, pkNewAddress, 100000000, System.currentTimeMillis(), None, smartMinFee, Array())
        .explicitGet()
        .json(),
      waitForTx = true
    )
    ...
    sender.signedBroadcast(
      TransferTransactionV2
        .selfSigned(Some(ByteStr.decodeBase58(asset).get), acc0, pkNewAddress, 800000000, System.currentTimeMillis(), None, smartMinFee, Array())
        .explicitGet()
        .json(),
      waitForTx = true
    )
