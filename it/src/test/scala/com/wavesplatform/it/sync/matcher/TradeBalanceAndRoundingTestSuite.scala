...
class TradeBalanceAndRoundingTestSuite extends MatcherSuiteBase {
  ...
  "Alice and Bob trade TN-USD" - {
    ...
    "place usd-TN order" in {
      ...
      matcherNode.reservedBalance(bobAcc)("TN") shouldBe sellOrderAmount + matcherFee
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("TN") shouldBe bobWavesBalanceBefore - (sellOrderAmount + matcherFee)
    ...
    "get opened trading markets. USD price-asset " in {
      ...
      markets.amountAssetName shouldBe "TN"
    ...
    "check reserved balance" in {
      ...
      matcherNode.reservedBalance(bobAcc)("TN") shouldBe expectedBobReservedBalance
    ...
    "check TN-usd tradable balance" in {
      ...
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("TN") shouldBe expectedBobTradableBalance
      matcherNode.tradableBalance(aliceAcc, wavesUsdPair)("TN") shouldBe aliceNode.accountBalances(aliceAcc.address)._1
      ...
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("TN") shouldBe bobNode.accountBalances(bobAcc.address)._1
    }
  }
  ...
  "Alice and Bob trade TN-USD check CELLING" - {
    ...
    "place usd-TN order" in {
      ...
      matcherNode.reservedBalance(bobAcc)("TN") shouldBe correctedSellAmount2 + matcherFee
      matcherNode.tradableBalance(bobAcc, wavesUsdPair)("TN") shouldBe bobWavesBalanceBefore - (correctedSellAmount2 + matcherFee)
  ...
  "Alice and Bob trade WCT-USD 1" - {
    ...
    "place wct-usd order" in {
      ...
      matcherNode.reservedBalance(bobAcc)("TN") shouldBe expectedReservedWaves
  ...
  "Alice and Bob trade WCT-TN on not enough fee when place order" - {
    ...
    "bob lease all  exact half matcher fee" in {
      ...
      matcherNode.tradableBalance(bobAcc, wctWavesPair)("TN") shouldBe matcherFee / 2 + receiveAmount(SELL, wctWavesSellAmount, wctWavesPrice) - matcherFee
  ...
  "Alice and Bob trade ETH-TN" - {
    "reserved balance is empty after the total execution" in {
      val counterId1 = matcherNode.placeOrder(aliceAcc, ethWavesPair, SELL, 2864310, 4000000, matcherFee).message.id
      matcherNode.waitOrderStatus(ethWavesPair, counterId1, "Accepted", 1.minute)

      val counterId2 = matcherNode.placeOrder(aliceAcc, ethWavesPair, SELL, 7237977, 4000000, matcherFee).message.id
      matcherNode.waitOrderStatus(ethWavesPair, counterId2, "Accepted", 1.minute)

      val submittedId = matcherNode.placeOrder(bobAcc, ethWavesPair, BUY, 4373667, 4000000, matcherFee).message.id
