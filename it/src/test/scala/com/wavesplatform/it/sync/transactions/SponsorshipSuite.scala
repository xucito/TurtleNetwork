...
class SponsorshipSuite extends FreeSpec with NodesFromDocker with Matchers with ReportingTestName with CancelAfterFailure {
  ...
  "Fee in sponsored asset works fine" - {
    ...
    val minerWavesBalanceAfterFirstXferTest   = minerWavesBalance + 2.TN + minWavesFee + 2 * leasingFee + Sponsorship.FeeUnit * SmallFee / Token
    val sponsorWavesBalanceAfterFirstXferTest = sponsorWavesBalance - 2.TN - minWavesFee - 2 * leasingFee - Sponsorship.FeeUnit * SmallFee / Token
