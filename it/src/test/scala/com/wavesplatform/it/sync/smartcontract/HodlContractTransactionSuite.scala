...
class HodlContractTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("set contract to contract account") {
    val scriptText =
      """
        ...
        |   if (isDefined(pmt.asset)) then throw("can hodl TN only at the moment")
        |   else {
  ...
  test("caller deposits TN") {
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          ...
          p = Some(ContractInvocationTransaction.Payment(1.5.TN, None)),
          timestamp = System.currentTimeMillis(),
          fee = 1.TN
        )
        .explicitGet()
    ...
    sender.getData(contract.address, caller.address) shouldBe IntegerDataEntry(caller.address, 1.5.TN)
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          sender = caller,
          contractAddress = contract,
          fc = FUNCTION_CALL(FunctionHeader.User("withdraw"), List(CONST_LONG(1.51.TN))),
          p = None,
          timestamp = System.currentTimeMillis(),
          fee = 1.TN
        )
  ...
  test("caller can withdraw less than he owns") {
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          sender = caller,
          contractAddress = contract,
          fc = FUNCTION_CALL(FunctionHeader.User("withdraw"), List(CONST_LONG(1.49.TN))),
          p = None,
          timestamp = System.currentTimeMillis(),
          fee = 1.TN
        )
    ...
    sender.getData(contract.address, caller.address) shouldBe IntegerDataEntry(caller.address, 0.01.TN)
