...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("not enough fee") {
    ...
    val setup = for {
      master <- accountGen
      ts     <- timestampGen
      genesis: GenesisTransaction = GenesisTransaction.create(master, 40000000000000L, ts).explicitGet()
      (issueTx, sponsorTx, _, _) <- sponsorFeeCancelSponsorFeeGen(master)
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          ...
          val expectedError =
            s"Fee for TransferTransaction (${insufficientFee.fee} in ${issue.assetId.base58})" ++
              s" does not exceed minimal vvalue of 2000000 TN  or $minFee ${issue.assetId.base58}"
  ...
  property("cannot cancel sponsorship") {
    ...
    val setup = for {
      ...
      ts         <- timestampGen
      genesis: GenesisTransaction = GenesisTransaction.create(master, 247370969792L, ts).explicitGet()
      (issueTx, sponsorTx, _, _) <- sponsorFeeCancelSponsorFeeGen(master)
      assetId = IssuedAsset(issueTx.id())
      senderNotIssuer = SponsorFeeTransaction
        .selfSigned(notSponsor, assetId, None, 10 * Constants.UnitsInWave, ts + 1)
        .right
        .get
      insufficientFee = SponsorFeeTransaction
        .selfSigned(notSponsor, assetId, None, 10 * Constants.UnitsInWave - 1, ts + 1)
        .right
        .get
  ...
  property("cannot сhange sponsorship fee") {
    ...
    val setup = for {
      ...
      notSponsor <- accountGen
      ts         <- timestampGen
      genesis: GenesisTransaction = GenesisTransaction.create(master, 400000000000L, ts).explicitGet()
      (issueTx, sponsorTx, _, _) <- sponsorFeeCancelSponsorFeeGen(master)
      assetId = IssuedAsset(issueTx.id())
      minFee <- smallFeeGen
      senderNotIssuer = SponsorFeeTransaction
        .selfSigned(notSponsor, assetId, Some(minFee), 10 * Constants.UnitsInWave, ts + 1)
        .right
        .get
      insufficientFee = SponsorFeeTransaction
        .selfSigned(master, assetId, Some(minFee), 10 * Constants.UnitsInWave - 1, ts + 1)
        .right
        .get
    } yield (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee)
